...
tail -n +2 "${SEED_CSV}" | while IFS=',' read -r project_id project_name owner repo rest; do
  ...
  # Skip repos that failed activity fetch (should not be any, JUST A CHECK)
  FAILED_JSON="${ROOT_DIR}/data/raw/github/_failed.json"
  if [ -f "${FAILED_JSON}" ] && grep -q "\"${repo}\"" "${FAILED_JSON}"; then
    echo "[skip] ${owner}/${repo} failed activity fetch, skipping."
    continue
  fi
